...
public class V2VM{
    ...
    public void start_instance_part2(int function_index_no){
        ...
        for(Map.Entry<String,Integer> label_entry: current_func_label_map.entrySet()){
            ...
            loop_instance_pt2(function_index_no,start_index,label_index,increment_on,add_another_increment);
            add_another_increment += 1;
            start_index = label_index;
            if(increment_on == false){
                increment_on = true;
            }
            System.out.println(label_name_map + ":");
        }
        if(label_index == list_instructions.length){
            loop_instance_pt2(function_index_no,label_index-1,list_instructions.length,increment_on,add_another_increment);
        }else{
            loop_instance_pt2(function_index_no,label_index,list_instructions.length,increment_on,add_another_increment);
        }
    ...
    public void big_print_function(){
        for(int i = 0 ; i < the_functions.length; i++){
            ...
            System.out.println("func " + test_function.ident);
            ...
            int local_t = 0;
            int local_s = 0;
            Map<Pair<Integer,String>,String> current_register_map = list_register_map.get(i);
            Iterator _itr = function_local_variables.iterator();
            while(_itr.hasNext()){
                String da_boss_string = (String)_itr.next();
                if(!da_boss_string.contains("t.")){
            ...
            temp_print_xyz.local_t = local_t;
            temp_print_xyz.local_s = local_s;
            temp_print_xyz.set_reg_map(current_register_map);
            start_instance_part2(i);
            System.out.println("Local: " + local_t  + " In: " + local_s);
        }
    ...
    public void loop_instance_pt2(int function_index,int start_index, int end_index, boolean increment_on, int map_value){
        ...
        for(int k = start_index; k < end_index; k++){
            temp_print_xyz.set_index(k + map_value);
